...
#include "cam_lidar_calibration/node_handle.h"
#include <ros/ros.h>
...
namespace cam_lidar_calibration
{
    ...
    public:
                ...
                : nh_("~")
        {
            ...
            {
                ROS_INFO_STREAM("Data received from /extrinsic_calib_param");
                param_msg_callback();
            } else
            {
        ...
        {
            std::cout << "\n";
            ROS_INFO("---- Calculating average reprojection error (model = %s) on %ld samples ---- \n", distortion_model.c_str(), sample_list.size());
        ...
        void import_samples(const fs::path &pose_path)
        {
            ...
            ROS_ASSERT_MSG(row.size() % sample_numrows == 0, "Invalid sample file");
            ...
            for (size_t i = 0; i < row.size(); i += sample_numrows)
            {
                ...
                temp.sample_num = static_cast<size_t>(row[i + sample_numrows - 1].x);
                sample_list.push_back(temp);
            }

            read_samples.close();

            auto n_samples_saved = cam_lidar_calibration::number_of_files_in_directory(data_dir / "images");
            ROS_ASSERT_MSG(n_samples_saved == sample_list.size(), "Number of samples in ,csv don't match saved images");

            n_samples_saved = cam_lidar_calibration::number_of_files_in_directory(data_dir / "pcd") / 2; // full and target is saved separately for each sample
            ROS_ASSERT_MSG(n_samples_saved == sample_list.size(), "Number of samples in .csv don't match saved images");

            ROS_INFO_STREAM(sample_list.size() << " samples imported from " << pose_path);
        }

        double
        compute_reprojection(const OptimisationSample &sample, std::vector<cv::Point2d> &cam,
                             std::vector<cv::Point2d> &lidar) const
        {
...
int main(int argc, char **argv)
{
    ...
    cam_lidar_calibration::AssessCalibration ac;
    ros::spin();
}
