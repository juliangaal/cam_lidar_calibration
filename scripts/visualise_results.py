...
def visualise_statistics(gauss, nbins_list, degree):
...
if __name__ == '__main__':
	...
	rospy.loginfo(f"Loaded {len(df)} samples from {path}")
	...
	# plt.show(block=False) shows black window, see https://github.com/matplotlib/matplotlib/issues/14873
	# start plot in a separate python process
	p = Process(target=visualise_statistics, args=(gauss, nbins_list, degree))
	p.start()

	publish_calibration(gauss)
